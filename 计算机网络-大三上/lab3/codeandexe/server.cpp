...
bool recvMessage(Message& recvMsg, SOCKET serverSocket, SOCKADDR_IN clientAddr)
{
	...
	while (1)
	{
		...
		if (recvByte > 0)
		{
			...
			//如果seq值不正确，说明接收到重复的报文，处理如下：
			//丢弃重复报文，重传该报文的回复ACK，实现就是只回复一个ack值对应接收消息的seq值的ACK报文
			//这种情况是因为上一个ACK报文丢失，从而使client超时重传
			else if (recvMsg.check() && (recvMsg.SeqNum != initrelseq + 1))
			{
				...
				replyMessage.setCheck();
				sendto(serverSocket, (char*)&replyMessage, sizeof(replyMessage), 0, (sockaddr*)&clientAddr, sizeof(SOCKADDR_IN));
				cout << "[重复接收报文] server收到 seq = " << recvMsg.SeqNum << "的数据报文，并发送 ack = " << replyMessage.AckNum << " 的ACK报文" << endl;
			}
		}
		else if (recvByte == 0)
		{
	...
	while (1)
	{
		...
		if (recvByte > 0)
		{
			...
			if (nameMessage.check() && (nameMessage.SeqNum == initrelseq + 1) && (nameMessage.flag & SFileName))
			{
				...
				cout << "server发送：seq = "<<replyMessage.SeqNum<<"，ack = " << replyMessage.AckNum << " 的ACK报文" << endl<<"装载文件名和文件大小的报文接收成功，下面开始正式传送数据段"<< endl<< endl;
				break;
			}
			...
			else if (nameMessage.check() && (nameMessage.SeqNum != initrelseq + 1) && (nameMessage.flag & SFileName))
			{
				...
				replyMessage.setCheck();
				sendto(serverSocket, (char*)&replyMessage, sizeof(replyMessage), 0, (sockaddr*)&clientAddr, sizeof(SOCKADDR_IN));
				cout << "[重复接收报文段]server收到 seq = " << nameMessage.SeqNum << "的数据报文，并发送 ack = " << replyMessage.AckNum << " 的ACK报文" << endl;
			}
		}
	}
