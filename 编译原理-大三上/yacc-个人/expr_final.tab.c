...
static const char *const yytname[] =
{
  ...
  "DIV", "LPT", "RPT", "NUMBER", "UMINUS", "';'", "$accept", "lines",
  "expr", YY_NULLPTR
};
...
#line 31 "expr_final.y"
                               {
                    printf("%s\n", yyvsp[-1]);
                }
#line 1092 "expr_final.tab.c"
    break;

  case 5: /* expr: expr ADD expr  */
#line 38 "expr_final.y"
                                {
                    int len = strlen(yyvsp[-2]) + strlen(yyvsp[0]) + 2;//+2 是为了包括最后的加号和字符串的终止字符 \0
                    yyval = (char*)malloc(len * sizeof(char));//这里我选择了使用精确分配内存，而不是直接预设一个较大的空间，这样可以更节省空间
                    strcpy(yyval, yyvsp[-2]);//把第一个expr放入结果
                    strcat(yyval, yyvsp[0]); //把第二个expr追加到结果中
                    strcat(yyval, "+ ");//把加号追加进去，这地方后面加了一个空格，可以更清晰表达后缀表达式
                }
#line 1104 "expr_final.tab.c"
    break;

  case 6: /* expr: expr MINUS expr  */
#line 45 "expr_final.y"
                                  {
                    int len = strlen(yyvsp[-2]) + strlen(yyvsp[0]) + 2;
                    yyval = (char*)malloc(len * sizeof(char));
                    strcpy(yyval, yyvsp[-2]);
                    strcat(yyval, yyvsp[0]); 
                    strcat(yyval, "- ");
                }
#line 1116 "expr_final.tab.c"
    break;

  case 7: /* expr: expr MULT expr  */
#line 52 "expr_final.y"
                                { 
                    int len = strlen(yyvsp[-2]) + strlen(yyvsp[0]) + 2;
                    yyval = (char*)malloc(len * sizeof(char));
                    strcpy(yyval, yyvsp[-2]);
...
#line 1128 "expr_final.tab.c"
    break;

  case 8: /* expr: expr DIV expr  */
#line 59 "expr_final.y"
                                {
                    int len = strlen(yyvsp[-2]) + strlen(yyvsp[0]) + 2;
                    yyval = (char*)malloc(len * sizeof(char));
                    strcpy(yyval, yyvsp[-2]);
...
#line 1140 "expr_final.tab.c"
    break;

  case 9: /* expr: LPT expr RPT  */
#line 66 "expr_final.y"
                                { yyval=yyvsp[-1]; }
#line 1146 "expr_final.tab.c"
    break;

  case 10: /* expr: MINUS expr  */
#line 67 "expr_final.y"
                                          {// %prec 提升优先级
                    int len = strlen(yyvsp[0]) + 2;
                    yyval = (char*)malloc(len * sizeof(char));
                    strcpy(yyval,"-");//这里没有加上空格，是为了在后缀表达式中可以体现减号和单目运算符-的不同
                    strcat(yyval,yyvsp[0]);
                }
#line 1157 "expr_final.tab.c"
    break;

  case 11: /* expr: NUMBER  */
#line 73 "expr_final.y"
                        {//处理数字就是直接赋值
                    yyval= (char*)malloc(strlen(yyvsp[0]));
                    strcpy(yyval,yyvsp[0]);}
#line 1165 "expr_final.tab.c"
    break;


#line 1169 "expr_final.tab.c"
